...
& $csc /nologo /target:library /out:"$root\build\Helper.Core.dll" @refs "$root\src\Core.cs" "$root\src\Markdown.cs"
...
if (Test-Path -LiteralPath "$root\src\Program.cs") {
    & $csc /nologo /target:winexe /optimize+ /platform:anycpu /win32manifest:"$root\app.manifest" /out:"$app" @refs "$root\src\Core.cs" "$root\src\Markdown.cs" "$root\src\GitHubClient.cs" "$root\src\MainForm.cs" "$root\src\Program.cs"
